...
## 12. Clone & Run from GitHub

### Quick start (fresh machine)

```bash
# 1. Clone the repository
git clone https://github.com/AmitChakor/HomeGenie.git
cd HomeGenie
...
Then scan the QR code with **Expo Go** (Android/iOS) to see the app on your phone.

### Full development build (for native features like camera & biometrics)
...
# 5. Install the APK on your phone, then:
npx expo start --dev-client
```

### What's NOT in the repo (and how to get it)

| Item | Why excluded | How to set up |
|------|-------------|---------------|
| `node_modules/` | Generated (200MB+) | `npm install` |
| `.env` | Contains secrets | Copy `.env.example` → `.env`, fill in values |
| `.expo/` | Metro cache | Auto-created on `npx expo start` |
| `ios/` / `android/` | Native builds | Generated by `eas build` or `npx expo prebuild` |
| `google-service-account.json` | Play Store secret | Download from Google Cloud Console |
| Signing keys (`.jks`, `.p12`) | Certificates | Managed by EAS Build automatically |

### Repository

**GitHub**: https://github.com/AmitChakor/HomeGenie
